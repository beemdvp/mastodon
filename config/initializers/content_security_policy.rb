...
Rails.application.config.content_security_policy do |p|
  ...
  if Rails.env.development?
    ...
    p.connect_src :self, :data, :blob, *media_hosts, Rails.configuration.x.streaming_api_base_url, *webpacker_urls, "localhost:4000"
    p.script_src  :self, :unsafe_inline, :unsafe_eval, assets_host
  else
    p.connect_src :self, :data, :blob, *media_hosts, Rails.configuration.x.streaming_api_base_url, "localhost:4000"
    p.script_src  :self, assets_host, "'wasm-unsafe-eval'", "localhost:4000"
  end
end
